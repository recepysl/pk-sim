...
public class AccountService : BaseService<Account, AccountRequest, AccountResponse>, IAccountService
{
    ...
    public ApiResponse<List<AccountResponse>> ByCustomerId(int userId)
    {
        if (userId == 0)
            return new ApiResponse<List<AccountResponse>>("Invalid User ID");

        try
        {
            var entityList = unitOfWork.Repository<Account>().Where(x => x.UserId == userId).ToList();
